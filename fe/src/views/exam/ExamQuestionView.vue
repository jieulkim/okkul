...
const updateSttDisplay = () => {
  const part1 = whisperTranscript.value.trim();
  const part2 = webSpeechPreviewTranscript.value.trim();
  
  let combined = part1;
  if (part2) combined += (combined ? " " : "") + part2;
  
  sttResult.value = combined;
  finalTranscriptAccumulated.value = part1 ? `${part1} ` : "";
};
...
const finalizeWhisperBeforeSubmit = async () => {
  const socket = whisperSocket.value;
  if (!socket) return;

  console.log("[STT] finalize start", {
  ...
  if (whisperDonePromise) {
    let doneByMessage = false;
    const doneWrapped = whisperDonePromise.then(() => {
      doneByMessage = true;
    });
    await Promise.race([
      doneWrapped,
    ...
    console.log("[STT] finalize wait finished", { doneByMessage });
  }
...
const initializeExam = async () => {
  try {
    ...
    if (savedData) {
      ...
      totalExamTime.value = data.totalExamTime || 0; // 경과 시간 로드
...
const startRecording = async () => {
  try {
    ...
    if (!wsUrl) {
      console.error("VITE_WHISPER_WS_URL is missing!");
    }
    console.log("[STT] start recording session", { wsUrl });
    whisperSocket.value = new WebSocket(wsUrl);
    whisperSocket.value.onopen = () => console.log("[WS] Exam: Connected");
      ...
      try {
        ...
        if (data.type === 'full' && data.text) {
          if (typeof data.seq === 'number') {
            if (data.seq <= lastWhisperSeq) return;
            lastWhisperSeq = data.seq;
          }
          whisperTranscript.value += data.text + " ";
          console.log("[WS] <- full", {
            seq: data.seq ?? null,
            segmentChars: data.text.length,
            totalWhisperChars: whisperTranscript.value.length,
          });
          webSpeechPreviewTranscript.value = "";
          updateSttDisplay();
        } else if (data.type === 'done') {
          whisperDoneReceived = true;
          console.log("[WS] <- done", { seq: data.seq ?? null });
          if (whisperDoneResolve) whisperDoneResolve();
          updateSttDisplay();
          if (isAwaitingFinalAfterEos && wsFinalizeResolver) {
            wsFinalizeResolver();
            wsFinalizeResolver = null;
            isAwaitingFinalAfterEos = false;
          }
          return;
        }
        ...
        if (data.type === 'error' && wsFinalizeResolver) {
          wsFinalizeResolver();
          wsFinalizeResolver = null;
          isAwaitingFinalAfterEos = false;
        }
    ...
    audioProcessor.onaudioprocess = (e) => {
      if (whisperSocket.value && whisperSocket.value.readyState === WebSocket.OPEN) {
    ...
    recognition.onresult = (event) => {
      ...
      for (let i = event.resultIndex; i < event.results.length; ++i) {
        webPreview += event.results[i][0].transcript + ' ';
      }
      webSpeechPreviewTranscript.value = webPreview.trim();
      if (event.resultIndex === 0) {
        console.log("[WebSpeech] preview update", {
          previewChars: webSpeechPreviewTranscript.value.length,
        });
      }
      updateSttDisplay();
    ...
    mediaRecorder.onstop = async () => {
      ...
      await finalizeWhisperBeforeSubmit();
      await submitAnswer(audioBlob);
    };
    
    // Volume Meter
    const analyser = audioContext.createAnalyser(); // Reuse context
...
const stopRecording = () => {
  if (mediaRecorder && isRecording.value) {
    console.log("[STT] stop requested", {
      ...
      currentWhisperChars: whisperTranscript.value.length,
      currentPreviewChars: webSpeechPreviewTranscript.value.length,
    });
    mediaRecorder.stop();
    ...
    // audioContext는 volume meter 등에서도 쓸 수 있으니 닫지 않거나, 여기선 닫아도 됨.
    // startRecording에서 매번 새로 만드므로 닫는 게 안전.
    if (audioContext) {
...
const submitAnswer = async (audioBlob) => {
  try {
    ...
    const normalizedWhisper = normalizeTranscript(whisperTranscript.value);
    console.log("[STT] submit payload", {
      whisperCharsRaw: whisperTranscript.value.length,
      whisperCharsNormalized: normalizedWhisper.length,
      recordingTime: recordingTime.value,
      audioBlobSize: audioBlob.size,
    });
    formData.append('file', audioBlob, 'answer.webm');
    formData.append('sttText', normalizedWhisper);
...
const saveProgress = () => {
  const data = {
    ...
    totalExamTime: totalExamTime.value
  };
  ...
  localStorage.setItem('incompleteExam', JSON.stringify({
    ...
    elapsedTime: formattedTotalTime.value, // remainingTime -> elapsedTime 명칭 변경
    isSubmitted: isSubmitted.value,
    currentIndex: currentQuestionIndex.value,
    totalQuestions: totalQuestions.value
  }));
        ...
        <section class="recording-section">
          <div class="recording-card">
            ...
            <div class="stt-box">
              ...
              <p v-if="!finalTranscript && !partialTranscript" class="stt-placeholder">
                말하면 partial/final 전사가 실시간으로 표시됩니다.
              </p>
            </div>
