...
async def analyze_integrated_score_gemini(request: QuestionAnalysisRequest, corrected_text: str) -> dict:
    ...
    [평가 항목]
    1. Grammar (grammar_score): 문법의 정확성, 시제 일치, 문장 구조의 복잡성
    2. Vocab (vocab_score): 어휘의 다양성, 적절한 이디엄 및 표현 사용
    3. Logic (logic_score): 답변의 논리적 흐름, 서론-본론-결론 구조, 근거 제시
    4. Fluency (fluency_score): 발화의 유창성, 다양한 어휘 사용, 자연스러운 필러를 사용했는지 등
    5. Relevance (relevance_score): 질문 의도에 대한 적합성, 동문서답 여부
    ...
    [작성 규칙]
    - improved_answer: 1차 교정 텍스트를 반드시 똑같이 사용하되, 접속사, 필러 등으로 흐름이 자연스러운 완벽한 AL 답안으로 작성하세요.
    - logic_feedback, fluency_feedback, relevance_feedback: 원본 답변을 기준으로 한국어로 상세 조언을 작성하세요.
    """
    ...
    payload = {
        ...
        "generationConfig": {
            ...
            "response_schema": {
                ...
                "required": ["grammar_score", "vocab_score", "logic_score", "fluency_score", "relevance_score", "improved_answer", "logic_feedback", "fluency_feedback", "relevance_feedback"]
            }
        }
    }
    headers = {"Content-Type": "application/json", "x-goog-api-key": GMS_KEY}
...
async def analyze_exam_integrated(request: QuestionAnalysisRequest):
    try:
        ...
        return QuestionScoreResponse(
            ...
            sentence_feedbacks=[SentenceFeedback(**s) for s in sentence_feedbacks_raw],
            **overall_result
        )
