...
// route 변화 감지 (쿼리 파라미터 변화도 감지)
watch(() => route.fullPath, (newPath, oldPath) => {
  // /practice 경로이고, 쿼리 파라미터가 없으면 초기화
  if (route.path === '/practice' && !route.query.type && !route.query.surveyId) {
    resetToInitialState()
  }
})
...
onMounted(async () => {
  ...
  // URL 쿼리 파라미터 확인 (설문 완료 후 돌아온 경우에만 처리)
  ...
  if (type && surveyId) {
    ...
    activeSurveyId.value = Number(surveyId);
    await useSelectedSurvey(Number(surveyId));
  } else {
    // 쿼리 파라미터가 없으면 초기 상태 유지
    resetToInitialState()
  }
});
...
<template>
  <div class="page-container">
    <main class="page-content">
      <div v-if="currentStep === 'type'" class="hero-section fade-in">
      ...
      <div v-else-if="currentStep === 'topic-check'">
