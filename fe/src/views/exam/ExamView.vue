...
const resumeExam = () => {
  if (incompleteExam.value) {
    showResumeModal.value = false; // 네비게이션 가드 통과를 위해 모달 먼저 닫기
    router.push({
      path: "/exam/question",
      query: {
...
onBeforeRouteLeave((to, from, next) => {
  if (showResumeModal.value || showSurveySelectModal.value) {
    // 모달이 켜져있으면 이동을 막음
    next(false);
  } else {
    next();
  ...
  <div class="page-container">
    ...
    <div v-if="showResumeModal" class="modal-overlay fade-in">
      <div class="modal-card resume-modal">
        <button @click="showResumeModal = false" class="modal-close-btn">
        ...
        <div class="resume-info" v-if="incompleteExam">
          ...
          <div class="info-row">
            <span class="label">진행 시간</span>
            <span class="value">{{ incompleteExam.elapsedTime || incompleteExam.remainingTime || "00:00" }}</span>
          </div>
        </div>
